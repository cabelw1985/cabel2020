...
def checkSkin():
	...
	if not DEFAULTSKIN == '':
		if os.path.exists(os.path.join(ADDONS, DEFAULTSKIN)):
			if DIALOG.yesno(ADDONTITLE, "[COLOR lime]Cini se da je skin ponovno postavljen[/COLOR] [COLOR white]%s[/COLOR]" % ( SKIN[5:].title()), "[COLOR lime]Zelite li vratiti skin na:[/COLOR]", '[COLOR white]%s[/COLOR]' % (DEFAULTNAME)):
	...
	if DEFAULTSKIN == '':
		...
		if len(skinlist) > 0:
			if len(skinlist) > 1:
				if DIALOG.yesno(ADDONTITLE, "[COLOR lime]Cini se da je skin ponovno postavljen[/COLOR] [COLOR white]%s[/COLOR]" % (SKIN[5:].title()), "[COLOR lime]Zelite li vidjeti popis dostupnih skinova?[/COLOR]"):
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR lime]Cini se da je skin ponovno postavljen[/COLOR] [COLOR white]%s[/COLOR]" % (SKIN[5:].title()), "[COLOR lime]Zelite li vratiti skin na:[/COLOR]", '[COLOR white]%s[/COLOR]' % (skinname[0])):
					gotoskin = skinlist[0]
					gotoname = skinname[0]
				else: wiz.log("Skin was not reset", xbmc.LOGNOTICE); wiz.setS('defaultskinignore', 'true')
