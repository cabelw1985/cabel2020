...
def updateLogin(do, who):
	...
	if do == 'update':
		...
		#else: wiz.LogNotify(name,'login Data: [COLOR lime]Not Found![/COLOR]', 2000, icon)
	elif do == 'clearaddon':
	...
	if who == 'all':
		...
		if os.path.exists(LOGINID[who]['path']):
			...
			if u == None or u == '': return
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to save the [COLOR %s]Login[/COLOR] data for [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "Addon: [COLOR green][B]%s[/B][/COLOR]" % u, "Saved:[/COLOR] [COLOR lime][B]%s[/B][/COLOR]" % su if not su == '' else 'Saved:[/COLOR] [COLOR lime][B]None[/B][/COLOR]', yeslabel="[B][COLOR green]Save Data[/COLOR][/B]", nolabel="[B][COLOR lime]No Cancel[/COLOR][/B]"):
					loginIt('update', who)
