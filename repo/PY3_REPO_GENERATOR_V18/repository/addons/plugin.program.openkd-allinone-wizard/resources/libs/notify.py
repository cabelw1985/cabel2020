...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font12',BorderWidth=10):
			...
			header = '[COLOR darkcyan][B]Advanced Settings Konfigurator[/B][/COLOR]'
			...
			header = '[COLOR darkcyan]Video Cache Groesse[/COLOR]'
			...
			msg3 = "[COLOR %s]Anzahl der Bytes zum Puffern im RAM.  Wird dies auf [COLOR %s]0[/COLOR] gesetzt, wird der Cache auf die Disk geschrieben anstatt in den RAM.  Info: Kodi braucht 3x soviel an freiem RAM des hier gewaehlten Wertes. Das kann bei falscher Einstellung zum Absturz von KODI fuehren. (1/3 of Freier RAM: [COLOR %s]%s[/COLOR])[/COLOR]" % (COLOR2, COLOR1, COLOR1, freeMemory)
			...
			header = '[COLOR darkcyan]CURL Timeout/CURL Low Speed[/COLOR]'
			...
			header = '[COLOR darkcyan]Read Buffer Faktor[/COLOR]'
			...
			msg3 = "[COLOR %s]Der Wert dieser Einstellung ist ein Multiplikator der standard Einstellung. Wen KODI eine normale Bluray Raw Datei mit 36 Mbit/s laed, benoetigt ein Wert von 2 eine Bandbreite von 72 Mbit/s. Hier kann im Vergleich zum RAM ein zu hoher Wert KODI nicht zum Absturz bringen.[/COLOR]" % COLOR2
			...
			header = '[COLOR darkcyan]Buffer Mode[/COLOR]'
			...
			self.addControl(self.Header4)
			msg4 = "[COLOR %s]Diese Einstellung zwingt KODI einen Cache fur alle Video Dateien zu verwenden. Der standard Wert ist 0 und buffert nur Internet Quellen.[/COLOR]" % COLOR2
			...
			self.buttonWrite=xbmcgui.ControlButton(leftside,T+H-40-BorderWidth,(W/2)-(BorderWidth*2),35,"Schreibe Datei",textColor="white",focusedColor="white",alignment=2,focusTexture=buttonfocus,noFocusTexture=buttonnofocus)
			self.buttonCancel=xbmcgui.ControlButton(rightside+BorderWidth*2,T+H-40-BorderWidth,(W/2)-(BorderWidth*2),35,"Schliessen",textColor="white",focusedColor="white",alignment=2,focusTexture=buttonfocus,noFocusTexture=buttonnofocus)
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es ist aktuell eine [COLOR %s]AdvancedSettings.xml[/COLOR] vorhanden, willst du diese entfernen und fortfahren?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLORlime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]")
				if choice == 0: return
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Aktuell ist kein Build von %s installiert.[CR][CR]Waehle '[B][COLOR darkcyan]Build Menu[/COLOR][/B]' um ein Build zu installieren oder '[B][COLOR darkcyan]Ignorieren[/COLOR][/B]' um den Hinweis zu schliessen.\n\nDanke das du den  %s verwendest." % (ADDONTITLE, ADDONTITLE)
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "[COLORwhite]Es gibt ein [COLORlime][B]Update[/B][/COLOR] des installierten Builds:\n[COLOR darkcyan]%s[/COLOR]\n\nAktuelle Version: [COLOR darkcyan]%s[/COLOR]\nNeuste Version: [COLOR darkcyan]%s[/COLOR]\n\n *Empfohlen:[/COLOR] [COLORlime][B]Frische Installation[/B][/COLOR]" % (self.name, self.current, self.new)
			self.msgcurrent = "[COLORwhite]Es ist die neuste Version des Builds installiert:\n[COLOR darkcyan]%s[/COLOR]\n\nAktuelle Version: [COLOR darkcyan]%s[/COLOR]\nNeuste Version: [COLOR darkcyan]%s[/COLOR]\n\n *Empfohlen: [COLORlime][B]Frische Installation[/B][/COLOR]" % (self.name, self.current, self.new)
...
################################################################################
#   Translated an modified by SGK @ Kodi Unlimited Support - Group / 02.2019   #
################################################################################	