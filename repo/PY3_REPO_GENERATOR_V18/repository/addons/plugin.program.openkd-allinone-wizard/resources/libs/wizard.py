...
def highlightText(msg):
	...
	for item in matches:
		...
		msg    = msg.replace(string, '[COLORred]%s[/COLOR]' % string)
	msg = msg.replace('WARNING', '[COLOR yellow]WARNUNG[/COLOR]').replace('ERROR', '[COLORred]FEHLER[/COLOR]').replace('[NL]', '\n').replace(': EXCEPTION Thrown (PythonToCppException) :', '[COLORred]: EXCEPTION Thrown (PythonToCppException) :[/COLOR]')
...
def getCacheSize():
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDEEXODUSREDUX == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.exodusredux', 'cache.db'))
			if INCLUDEYODA == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.Yoda', 'cache.db'))
	...
	if os.path.exists(dep):
		...
		for depends in match:
			if not 'xbmc.python' in depends:
				...
				if not DP == None: 
					DP.update("","Checke Abhangigkeiten [COLORyellow]%s[/COLOR] von [COLOR yellow]%s[/COLOR]" % (depends, name),"")
				if os.path.exists(dependspath):
					toggleAddon(name, 'true')
			xbmc.sleep(100)

def toggleAdult():
	do = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die Adult Addons [COLOR %s]Aktivieren[/COLOR] oder [COLOR %s]Deaktivieren[/COLOR] ?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLORlime]Aktivieren[/COLOR][/B]", nolabel="[B][COLORred]Deaktivieren[/COLOR][/B]")
	...
	if len(found) > 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du eine Liste aller Addons sehen die %s sind?[/COLOR]" % (COLOR2, goto.replace('ing', 'ed')), yeslabel="[B][COLORlime]JA, anzeigen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
...
def createQR():
	...
	if not working == True:
		if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Deine URL funnktioniert nicht, willst du trotzdem fortfahren?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLORred]Ja, erstellen[/COLOR][/B]", nolabel="[B][COLORlime]NEIN, abbrechen[/COLOR][/B]"):
			return
...
def cleanupBackup():
	...
	if selected == -1:
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du alle Dateien im 'My_Builds' Ordner entfernen?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLORlime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]%s[/COLOR] vom 'My_Builds' Ordner entfernen?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLORlime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
...
def clearCrash():  
	...
	for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
		files.append(file)
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du die Crash Logs entfernen?' % COLOR2, '[COLOR %s]%s[/COLOR] Dateien gefunden[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLORlime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
			for f in files:
				os.remove(f)
...
def hidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du alle Passworteingaben in den Addon Einstellungen [COLOR %s]verbergen[/COLOR] ?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLORlime]JA, verbergen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
		count = 0
...
def unhidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du alle Passworteingaben in den Addon Einstellungen [COLOR %s]anzeigen[/COLOR] ?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLORlime]JA, anzeigen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
		count = 0
		for folder in glob.glob(os.path.join(ADDONS, '*/')):
			sett = os.path.join(folder, 'resources', 'settings.xml')
			if os.path.exists(sett):
...
def wizardUpdate(startup=None):
	if workingURL(WIZARDFILE):
		...
		zip = checkWizard('zip')
		if ver > VERSION:
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Es gibt eine neue Version von [COLOR %s]%s[/COLOR]!' % (COLOR2, COLOR1, ADDONTITLE), 'Willst du sie herunter laden [COLOR %s]v%s[/COLOR]?[/COLOR]' % (COLOR1, ver), nolabel='[B][COLORred]NEIN, abbrechen[/COLOR][/B]', yeslabel="[B][COLORlime]JA, Wizard updaten[/COLOR][/B]")
			if yes:
...
def asciiCheck(use=None, over=False):
	if use == None:
		...
		if over == True:
			...
			yes = DIALOG.yesno(ADDONTITLE,'[COLOR %s]Willst du alle Dateinamen mit spezial Zeichen [COLOR %s]entfernen[/COLOR] oder willst du nur [COLOR %s]scannen[/COLOR] und das Ergebnis im Log anzeigen?[/COLOR]' % (COLOR2, COLOR1, COLOR1), yeslabel='[B][COLORlime]JA, entfernen[/COLOR][/B]', nolabel='[B][COLORred]NEIN, nur scannen[/COLOR][/B]')
	else: 
		source = use
		yes = 1
...
def backUpOptions(type, name=""):
	...
	try:
		...
		return
	if type == "build":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du das aktuelle Build sichern?[/COLOR]" % COLOR2, nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]", yeslabel="[B][COLORlime]JA, Backup erstellen[/COLOR][/B]"):
			if name == "":
			...
			try:
				...
				try:
					...
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]In das aktuelle Verzeichnis kann nicht geschrieben werden, willst du das Verzeichnis wechseln?[/COLOR]" % COLOR2, yeslabel="[B][COLORlime]JA, Verzeichnis wechseln[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
						openS()
						return
					else:
						return
		...
		if not DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du ein Theme Backup erstellen?[/COLOR]" % COLOR2, yeslabel="[B][COLORlime]JA, erstellen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"): LogNotify("Theme Backup", "Cancelled!"); return False
		if name == "":
		...
		try:
			if not SKIN == 'skin.confluence':
				...
				if len(match2) > 1:
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die Texture Datei mit sichern?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
						skinfold = os.path.join(ADDONS, SKIN, 'media')
						match2 = glob.glob(os.path.join(skinfold,'*.xbt'))
						for xbt in match2:
							if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du diese Texture Datei sichern [COLOR %s]%s[/COLOR]?" % (COLOR1, COLOR2, xbt.replace(skinfold, "")[1:]), "from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
								fn  = xbt
								fn2 = fn.replace(HOME, "")
								zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				else:
					for xbt in match2:
						if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du diese Texture Datei sichern [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, xbt.replace(skinfold, "")[1:]), "from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
							fn  = xbt
							fn2 = fn.replace(HOME, "")
							zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				ad_skin = os.path.join(ADDOND, SKIN, 'settings.xml')
				if os.path.exists(ad_skin):
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die [COLOR %s]settings.xml[/COLOR] in [COLOR %s]/addon_data/[/COLOR] mit sichern?" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]"  % (COLOR1, SKIN), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
				...
				if 'script.skinshortcuts' in match:
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die [COLOR %s]settings.xml[/COLOR] von [COLOR %s]script.skinshortcuts[/COLOR] mit sichern?" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
						for base, dirs, files in os.walk(os.path.join(ADDOND,'script.skinshortcuts')):
							files[:] = [f for f in files if f not in exclude_files]
							for file in files:
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du den [COLOR %s]Backgrounds[/COLOR] Ordner mit sichern?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
				...
				if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du [COLOR %s]%s[/COLOR] mit sichern?[/COLOR]" % (COLOR2, COLOR1, text), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
					zipf.write(os.path.join(DATABASE, text), '/userdata/Database/%s' % text, zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du Addons mit sichern?[/COLOR]" % (COLOR2), yeslabel="[B][COLORlime]JA[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
					...
					for item in selected:
						for base, dirs, files in os.walk(os.path.join(ADDONS,addonfolds[item])):
							...
							for file in files:
								...
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die [COLOR %s]guisettings.xml[/COLOR] mit sichern?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLORlime]JA, sichern[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du den aktuellen Addon_Data Ordner sichern?[/COLOR]" % COLOR2, nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]", yeslabel="[B][COLORlime]JA, Addon_Data sichern[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Gib einen Namen ein fur die %s zip" % type)
				if not name: return False
...
def restoreLocal(type):
	...
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, fn), 'Fertig: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLORred]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLORlime]JA, zeige Fehler[/COLOR][/B]')
		if yes:
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder das [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLORred]Profil neu laden[/COLOR][/B]", nolabel="[B][COLORlime]KODI schliessen[/COLOR][/B]")
	if todo == 1: reloadFix()
	else: killxbmc(True)
...
def restoreExternal(type):
	...
	lookandFeelData('save')
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, zname), 'Fertig: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLORred]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLORlime]Ja, zeige Fehler[/COLOR][/B]')
		if yes:
			TextBox(ADDONTITLE, error.replace('\t',''))
	setS('installed', 'true')
	setS('extract', str(percent))
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder das [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLORred]Profile neu laden[/COLOR][/B]", nolabel="[B][COLORlime]Kodi schliessen[/COLOR][/B]")
	if todo == 1: reloadFix()
	else: killxbmc(True)
...
def whiteList(do):
	...
	if   do == 'edit':
		...
		try:
			...
			if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]In das aktuelle Verzeichnis kann nicht geschriebn werden, willst du das Verzeichnis wechseln?[/COLOR]" % COLOR2, yeslabel="[B][COLORlime]JA, Verzeichnis wechseln[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
				LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Whitelist Export Cancelled[/COLOR]" % (COLOR2, e))
			else:
				whitelist(export)
	elif do == 'clear':
		if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die WhiteList bereinigen?" % COLOR2, "Dieser Prozess kann nicht ruckgangig gemacht werden.[/COLOR]", yeslabel="[B][COLORlime]JA, bereinigen[/COLOR][/B]", nolabel="[B][COLORred]NEIN, abbrechen[/COLOR][/B]"):
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Reinige Package Dateien" % COLOR2, "[COLOR %s]%s[/COLOR] Dateien gefunden / [COLOR %s]%s[/COLOR] Gesamtgrosse." % (COLOR1, str(file_count), COLOR1, size), "Willst du die Dateien bereinigen?[/COLOR]", nolabel='[B][COLORred]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLORlime]JA, Dateien reinigen[/COLOR][/B]')
					if yes:
...
def checkSources():
	...
	if len(match) > 0:
		...
		if len(bad) > 0:
			choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] fehlerhafte Quellen wurden gefunden" % (COLOR1, len(bad), COLOR2),"Willst du alle oder einzel entfernen?[/COLOR]", yeslabel="[B][COLORlime]Alle entfernen[/COLOR][/B]", nolabel="[B][COLORred]Einzeln entfernen[/COLOR][/B]")
			if choice == 1:
				remove = bad
			else:
				for name, path, sharing, working in bad: 
					log("%s sources: %s, %s" % (name, path, working), xbmc.LOGNOTICE)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] wurde als fehlerhaft gemeldet" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLORlime]Quelle entfernen[/COLOR][/B]", nolabel="[B][COLORred]Quelle behalten[/COLOR][/B]"):
...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('KODI schliessen', '[COLOR %s]Du bist dabei KODI zu schliessen' % COLOR2, 'Willst du fortfahrene?[/COLOR]', nolabel='[B][COLORred]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLORlime]JA, KODI schliessen[/COLOR][/B]')
	if choice == 1:
