...
def updateAlluc(do, who):
	...
	if do == 'update':
		...
		#else: wiz.LogNotify(name,'Alluc Data: [COLOR lime]Not Found![/COLOR]', 2000, icon)
	elif do == 'clearaddon':
	...
	if who == 'all':
		...
		if os.path.exists(ALLUCID[who]['path']):
			...
			if u == None or u == '': return
				...
				if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to save the [COLOR %s]Alluc[/COLOR] data for [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "Addon: [COLOR green][B]%s[/B][/COLOR]" % u, "Saved:[/COLOR] [COLOR lime][B]%s[/B][/COLOR]" % su if not su == '' else 'Saved:[/COLOR] [COLOR lime][B]None[/B][/COLOR]', yeslabel="[B][COLOR green]Save Data[/COLOR][/B]", nolabel="[B][COLOR lime]No Cancel[/COLOR][/B]"):
					allucIt('update', who)
...
def importlist(who):
	if who == 'all':
		...
		if os.path.exists(ALLUCID[who]['file']):
			...
			if len(m) > 0:
				if not m[0] == su:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to import the [COLOR %s]Alluc[/COLOR] data for [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, COLOR1, n), "File: [COLOR green][B]%s[/B][/COLOR]" % m[0], "Saved:[/COLOR] [COLOR lime][B]%s[/B][/COLOR]" % su if not su == '' else 'Saved:[/COLOR] [COLOR lime][B]None[/B][/COLOR]', yeslabel="[B][COLOR green]Save Data[/COLOR][/B]", nolabel="[B][COLOR lime]No Cancel[/COLOR][/B]"):
						wiz.setS(sa, m[0])
