...
def index():
	...
	err = str(errors)
	errorsfound = '[COLOR lime]%s[/COLOR] Error(s) Found'  % (err) if errors > 0 else 'None Found'
	if AUTOUPDATE == 'Yes':
		wizfile = wiz.textCache(WIZARDFILE)
		if not wizfile == False:
			ver = wiz.checkWizard('version')
			if ver > VERSION: addFile('%s [v%s] [COLOR lime][B][UPDATE v%s][/B][/COLOR]' % (ADDONTITLE, VERSION, ver), 'wizardupdate', themeit=THEME2)
	...
	if len(BUILDNAME) > 0:
		...
		if version > BUILDVERSION: build = '%s [COLOR lime][B][UPDATE v%s][/B][/COLOR]' % (build, version)
...
def buildMenu():
	...
	addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
	addDir ('Save Data Menu'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
	addDir ('[COLOR yellow]---[B][COLOR lime]Addon Packs [COLOR blue]/ [COLOR lime]Fixes[/COLOR][/B][COLOR yellow]---[/COLOR]'        ,'viewpack',   icon=ICONMAINT,   themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	if third == True:
		for item in addin:
...
def viewBuild(name):
	...
	for version, url, url2, url3, gui, kodi, themefile, icon, fanart, adult, description, preview in match:
		...
		if BUILDNAME == name and version > BUILDVERSION:
			build = '%s [COLOR lime][CURRENT v%s][/COLOR]' % (build, BUILDVERSION)
...
def UNZIPROM():
	...
	if myroms == '':
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]It seems that you do not have an extract location setup for Rom Packs" % COLOR2, "Would you like to set one?[/COLOR]", yeslabel="[COLOR green][B]Set Location[/B][/COLOR]", nolabel="[COLOR lime][B]Cancel Download[/B][/COLOR]"):
			wiz.openS('rompath')
			myroms = wiz.getS('rompath')
			if myroms == '': return
	yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you would like to download and extract [COLOR %s]%s[/COLOR] to:" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, myroms), yeslabel="[B][COLOR green]Download[/COLOR][/B]", nolabel="[B][COLOR lime]Cancel[/COLOR][/B]")
				...
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						...
						if selected == 0:
							...
							if DIALOG.yesno(ADDONTITLE, "[COLOR lime]Zelite li ukloniti:"  "[COLOR white]%s[/COLOR][COLOR lime]?[/COLOR]" , yeslabel="[B][COLOR white]Nastavi[/COLOR][/B]", nolabel="[B][COLOR white]Odustani[/COLOR][/B]"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						...
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to install the repository for [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR green]Yes Install[/COLOR][/B]", nolabel="[B][COLOR lime]No Skip[/COLOR][/B]"): 
...
def maintMenu(view=None):
	on = '[B][COLOR green]ON[/COLOR][/B]'; off = '[B][COLOR lime]OFF[/COLOR][/B]'
	...
	wizlogsize = ': [COLOR lime]Not Found[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR green]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
...
def writeAdvanced(name, url):
	...
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to overwrite your current Advanced Settings with [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Overwrite[/COLOR][/B]", nolabel="[B][COLOR lime]Cancel[/COLOR][/B]")
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to download and install [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Install[/COLOR][/B]", nolabel="[B][COLOR lime]Cancel[/COLOR][/B]")
		if choice == 1:
...
def saveMenu():
	on = '[COLOR green]ON[/COLOR]'; off = '[COLOR lime]OFF[/COLOR]'
...
def FavsMenu():
	on = '[COLOR green]ON[/COLOR]'; off = '[COLOR lime]OFF[/COLOR]'
	fav = '[COLOR green]ON[/COLOR]' if KEEPFAVS == 'true' else '[COLOR lime]OFF[/COLOR]'
	...
	setView('files', 'viewType')
def traktMenu():
	trakt = '[COLOR green]ON[/COLOR]' if KEEPTRAKT == 'true' else '[COLOR lime]OFF[/COLOR]'
	...
	for trakt in traktit.ORDER:
		...
		if not os.path.exists(path): addFile('[COLOR lime]Addon Data: Not Installed[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR lime]Addon Data: Not Registered[/COLOR]','authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR green]Addon Data: %s[/COLOR]' % auser,'authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR lime]Saved Data: Save File Found(Import Data)[/COLOR]','importtrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR lime]Saved Data: Not Saved[/COLOR]','savetrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
	...
	setView('files', 'viewType')
def realMenu():
	real = '[COLOR green]ON[/COLOR]' if KEEPREAL == 'true' else '[COLOR lime]OFF[/COLOR]'
	...
	for debrid in debridit.ORDER:
		...
		addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME3)
		if not os.path.exists(path): addFile('[COLOR lime]Addon Data: Not Installed[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR lime]Addon Data: Not Registered[/COLOR]','authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR green]Addon Data: %s[/COLOR]' % auser,'authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR lime]Saved Data: Save File Found(Import Data)[/COLOR]','importdebrid', debrid, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR lime]Saved Data: Not Saved[/COLOR]','savedebrid', debrid, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR green]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	...
	setView('files', 'viewType')
def allucMenu():
	alluc = '[COLOR green]ON[/COLOR]' if KEEPALLUC == 'true' else '[COLOR lime]OFF[/COLOR]'
	...
	for alluc in allucit.ORDER:
		...
		addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME3)
		if not os.path.exists(path): addFile('[COLOR lime]Addon Data: Not Installed[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR lime]Addon Data: Not Registered[/COLOR]','authalluc', alluc, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR green]Addon Data: %s[/COLOR]' % auser,'authalluc', alluc, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR lime]Saved Data: Save File Found(Import Data)[/COLOR]','importalluc', alluc, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR lime]Saved Data: Not Saved[/COLOR]','savealluc', alluc, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR green]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	...
	setView('files', 'viewType')
def loginMenu():
	login = '[COLOR green]ON[/COLOR]' if KEEPLOGIN == 'true' else '[COLOR lime]OFF[/COLOR]'
	...
	for login in loginit.ORDER:
		...
		addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME3)
		if not os.path.exists(path): addFile('[COLOR lime]Addon Data: Not Installed[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR lime]Addon Data: Not Registered[/COLOR]','authlogin', login, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR green]Addon Data: %s[/COLOR]' % auser,'authlogin', login, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR lime]Saved Data: Save File Found(Import Data)[/COLOR]','importlogin', login, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR lime]Saved Data: Not Saved[/COLOR]','savelogin', login, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR green]Saved Data: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to [COLOR %s]Force close[/COLOR] kodi or [COLOR %s]Reload Profile[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Reload Profile[/COLOR][/B]", nolabel="[B][COLOR lime]Force Close[/COLOR][/B]")
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		addFile('[COLOR lime][B][REMOVE][/B][/COLOR] All Addon_Data', 'removedata', 'all', themeit=THEME2)
		addFile('[COLOR lime][B][REMOVE][/B][/COLOR] All Addon_Data for Uninstalled Addons', 'removedata', 'uninstalled', themeit=THEME2)
		addFile('[COLOR lime][B][REMOVE][/B][/COLOR] All Empty Folders in Addon_Data', 'removedata', 'empty', themeit=THEME2)
		addFile('[COLOR lime][B][REMOVE][/B][/COLOR] %s Addon_Data' % ADDONTITLE, 'resetaddon', themeit=THEME2)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			replace = {'audio.':'[COLOR orange][AUDIO] [/COLOR]', 'metadata.':'[COLOR cyan][METADATA] [/COLOR]', 'module.':'[COLOR orange][MODULE] [/COLOR]', 'plugin.':'[COLOR blue][PLUGIN] [/COLOR]', 'program.':'[COLOR orange][PROGRAM] [/COLOR]', 'repository.':'[COLOR white][REPO] [/COLOR]', 'script.':'[COLOR green][SCRIPT] [/COLOR]', 'service.':'[COLOR green][SERVICE] [/COLOR]', 'skin.':'[COLOR dodgerblue][SKIN] [/COLOR]', 'video.':'[COLOR orange][VIDEO] [/COLOR]', 'weather.':'[COLOR yellow][WEATHER] [/COLOR]'}
			for rep in replace:
			...
			else: folderdisplay = '[COLOR lime][B][REMOVE][/B][/COLOR] %s' % folderdisplay
	...
	setView('files', 'viewType')
def enableAddons():
	addFile("[I][B][COLOR lime]!!Notice: Disabling Some Addons Can Cause Issues!![/COLOR][/B][/I]", '', icon=ICONMAINT)
	fold = glob.glob(os.path.join(ADDONS, '*/'))
	x = 0
	for folder in sorted(fold, key = lambda x: x):
		foldername = os.path.split(folder[:-1])[1]
		...
		if os.path.exists(addonxml):
			...
			try:
				...
				state  = "[COLOR lime][Disabled][/COLOR]"
				goto   = "true"
				pass
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR lime]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR lime]WARNING!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build is not currently installed." % (COLOR2, COLOR1, name), "Would you like to apply the guiFix anyways?.[/COLOR]", nolabel='[B][COLOR lime]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		if yes:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Gui fix has been installed.  Would you like to Reload the profile or Force Close Kodi?[/COLOR]" % COLOR2, yeslabel="[B][COLOR lime]Reload Profile[/COLOR][/B]", nolabel="[B][COLOR green]Force Close[/COLOR][/B]")
			if todo == 1: wiz.reloadFix()
			else: DIALOG.ok(ADDONTITLE, "[COLOR lime]Za spremanje promijena trebate pirislino ugasiti Kodi. Pritisnike[/COLOR] [COLOR white]- OK -[/COLOR] [COLOR lime]za prisilno gasenje Kodi-a. Zatim ponovno pokrenite Kodi nakon nekoliko trenutaka.[/COLOR]"); wiz.killxbmc('true')
		else:
		...
		if warning == True:
			yes_pressed = DIALOG.yesno("%s - [COLOR lime]Upozorenje!![/COLOR]" % ADDONTITLE, '[COLOR lime]Postoji mogucnost da se presvlaka nece ispravno instalirati[/COLOR]' , '[COLOR lime]Kada instalirate[/COLOR] [COLOR white]%s[/COLOR] [COLOR lime]build na Kodi[/COLOR] [COLOR white]%s[/COLOR] [COLOR lime]instalaciju[/COLOR]' % (wiz.checkBuild(name, 'kodi'), KODIV), '[COLOR lime]Zelite li nastaviti instalaciju?[/COLOR] [COLOR white]%s v%s[/COLOR][COLOR lime]?[/COLOR]' % ( name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR white]Odustani[/COLOR][/B]',yeslabel='[B][COLOR white]Nastavi[/COLOR][/B]')
		else:
			if not over == False: yes_pressed = 1
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li preuzeti i instalirati[/COLOR]' , '[COLOR white]%s v%s[/COLOR][COLOR lime]?[/COLOR]' % (name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR white]Odustani[/COLOR][/B]',yeslabel='[B][COLOR white]Nastavi[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			#buildzips = []
			...
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR lime]Preuzimanje[/COLOR] [COLOR white]%s v%s[/COLOR]' % (name, wiz.checkBuild(name,'version')),'', 'Please Wait')
			lib=os.path.join(PACKAGES, '%s.zip' % zipname)
			try: os.remove(lib)
			except: pass
			downloader.download(buildzip, lib, DP)
			xbmc.sleep(500)
			title = '[COLOR lime]Instaliranje:[/COLOR] [COLOR white]%s v%s[/COLOR]' % (name, wiz.checkBuild(name,'version'))
			DP.update(0, title,'', 'Please Wait')
			percent, errors, error = extract.all(lib,HOME,DP, title=title)
			if int(float(percent)) > 0:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR lime]Za spremanje promijena trebate pirislino ugasiti Kodi. Pritisnike[/COLOR] [COLOR white]- OK -[/COLOR] [COLOR lime]za prisilno gasenje Kodi-a. Zatim ponovno pokrenite Kodi nakon nekoliko trenutaka.[/COLOR]"); wiz.killxbmc('true')
			else:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Build [COLOR %s]%s[/COLOR] comes with [COLOR %s]%s[/COLOR] different themes" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Would you like to install one now?[/COLOR]", yeslabel="[B][COLOR green]Install Theme[/COLOR][/B]", nolabel="[B][COLOR lime]Cancel Themes[/COLOR][/B]"):
		...
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to install the theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'for [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR green]Install Theme[/COLOR][/B]", nolabel="[B][COLOR lime]Cancel Themes[/COLOR][/B]")
		if installtheme:
...
def thirdPartyInstall(name, url):
	...
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to preform a [COLOR %s]Fresh Install[/COLOR] or [COLOR %s]Normal Install[/COLOR] for:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR green]Fresh Install[/COLOR][/B]", nolabel="[B][COLOR lime]Normal Install[/COLOR][/B]")
	if type == 1:
	...
	if int(float(percent)) > 0:
		...
		if int(float(errors)) > 0:
			yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Would you like to view the errors?[/COLOR]', nolabel='[B][COLOR lime]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR green]View Errors[/COLOR][/B]')
			if yes:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox(ADDONTITLE, error)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android'  or DEVELOPER == 'true':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to download and install:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR green]Download[/COLOR][/B]", nolabel="[B][COLOR lime]Cancel[/COLOR][/B]")
		if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]ERROR: Install Cancelled[/COLOR]' % COLOR2); return
		display = apk
		if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
...
def apkInstaller1(apk, url):
	if wiz.platform() == 'android' or DEVELOPER == 'true':
		...
		if yes:
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			if not wiz.workingURL(url) == True: wiz.LogNotify(ADDONTITLE, 'APK Installer: [COLOR lime]Invalid Apk Url![/COLOR]'); return
		...
		else: wiz.LogNotify(ADDONTITLE, '[COLOR lime]ERROR:[/COLOR] Install Cancelled')
	else: wiz.LogNotify(ADDONTITLE, '[COLOR lime]ERROR:[/COLOR] None Android Device')
...
def errorChecking(log=None, count=None, last=None):
	...
	if not count == None:
		...
		if last == None:
			...
			for item in errors:
				i += 1
				string += "[B][COLOR lime]ERROR NUMBER %s:[/B][/COLOR]%s\n" % (str(i), item.replace(HOME, '/').replace('                                        ', ''))
		else:
			string = "[B][COLOR lime]Last Error in Log:[/B][/COLOR]%s\n" % (errors[0].replace(HOME, '/').replace('                                        ', ''))
...
def log_tools():
	errors = int(errorChecking(count=True))
	err = str(errors)
	errorsfound = '[COLOR lime]%s[/COLOR] Found'  % (err) if errors > 0 else 'None Found'
	on = '[B][COLOR green]ON[/COLOR][/B]'; off = '[B][COLOR lime]OFF[/COLOR][/B]'
	if wiz.Grab_Log(True) == False: kodilog = 0
	else: kodilog = errorChecking(wiz.Grab_Log(True), True)
	if wiz.Grab_Log(True, True) == False: kodioldlog = 0
...
def removeAddon(addon, name, over=False):
	if not over == False:
		...
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Are you sure you want to delete the addon:'% COLOR2, 'Name: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Remove Addon[/COLOR][/B]', nolabel='[B][COLOR lime]Don\'t Remove[/COLOR][/B]')
	if yes == 1:
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		if DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li ukloniti[/COLOR] [COLOR white]sve[/COLOR] [COLOR lime]podatke o dodatcima u korisnickoj mapi?[/COLOR]' , yeslabel='[B][COLOR white]Uredu[/COLOR][/B]', nolabel='[B][COLOR white]Odustani[/COLOR][/B]'):
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li ukloniti[/COLOR] [COLOR white]sve[/COLOR] [COLOR lime]podatke o dodatcima spremljene u vasu Userdata mapu izbrisanih dodataka?[/COLOR]' , yeslabel='[B][COLOR white]Nastavi[/COLOR][/B]', nolabel='[B][COLOR white]Odustani[/COLOR][/B]'):
			total = 0
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li ukloniti[/COLOR] [COLOR white]sve[/COLOR] [COLOR lime] prazne dodatke mape u vasoj Userdata mapi?[/COLOR]' , yeslabel='[B][COLOR white]Nastavi[/COLOR][/B]', nolabel='[B][COLOR white]Odustani[/COLOR][/B]'):
		...
		if addon in EXCLUDES:
			...
			if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you also like to remove the addon data for:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR green]Remove Data[/COLOR][/B]', nolabel='[B][COLOR lime]Don\'t Remove[/COLOR][/B]'):
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like replace the current [COLOR %s]%s[/COLOR] file?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR green]Yes Replace[/COLOR][/B]", nolabel="[B][COLOR lime]No Skip[/COLOR][/B]"): continue
					else: os.remove(old)
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR lime]Odaberite [COLOR white] - Uredu - [/COLOR]" , "[COLOR lime]Za brisanje trenutnog build-a. \r\nZatim instalirajte lokalnu ili vanjsku verziju build-a.[/COLOR]", nolabel='[B][COLOR white]Odustani[/COLOR][/B]', yeslabel='[B][COLOR white]Uredu[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR lime]Odaberi [COLOR white] - Nastavi - [/COLOR]" , "[COLOR lime]Za brisanje trenutnog build-a, \r\nZatim svjeze instaliraj[/COLOR] [COLOR white]%s[/COLOR]!!" % ( install), nolabel='[B][COLOR white]Odustani[/COLOR][/B]', yeslabel='[B][COLOR white]Nastavi[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Do you wish to restore your" % COLOR2, "Configuration to default settings?[/COLOR]", nolabel='[B][COLOR lime]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Yes[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
			#yes=DIALOG.yesno(ADDONTITLE, "[COLOR %s]The skin needs to be set back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, skin[5:]), "Before doing a fresh install to clear all Texture files,", "Would you like us to do that for you?[/COLOR]", yeslabel="[B][COLOR springgreen]Switch Skins[/COLOR][/B]", nolabel="[B][COLOR lime]I'll Do It[/COLOR][/B]";
			#if yes:
		...
		DP.create(ADDONTITLE,"[COLOR lime]Pregled datoteka i mapa" ,'', '[COLOR lime]Pricekajte molim![/COLOR]')
		...
		DP.update(0, "[COLOR lime]Brisanje trenutnih mapa i datoteka:[/COLOR]")
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			...
			for name in files:
				...
				if name == 'sources.xml' and fold[-1] == 'userdata' and KEEPSOURCES == 'true': wiz.log("Keep Sources: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
					...
					DP.update(int(wiz.percentage(del_file, total_files)), '', '[COLOR lime]Datoteka:[/COLOR] [COLOR white]%s[/COLOR]' % (name), '')
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			dirs[:] = [d for d in dirs if d not in EXCLUDES]
			for name in dirs:
				DP.update(100, '', '[COLOR lime]Brisanje praznih mapa: [COLOR white]%s[/COLOR]' % (name), '')
...
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li obrisati predmemoriju?[/COLOR]' , nolabel='[B][COLOR white]Odustani[/COLOR][/B]', yeslabel='[B][COLOR white]Obrisi[/COLOR][/B]'):
		wiz.clearCache()
		DC.setLabel('Velicina: [B][COLOR lime]0.0 B[/B][/COLOR]')
def clearArchive():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to clear the \'Archive_Cache\' folder?[/COLOR]' % COLOR2, nolabel='[B][COLOR lime]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR green]Yes Clear[/COLOR][/B]'):
		wiz.clearArchive()
def clearPackages():
	if DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li obrisati pakete?[/COLOR]' , nolabel='[B][COLOR white]Odustani[/COLOR][/B]', yeslabel='[B][COLOR white]Obrisi[/COLOR][/B]'):
		wiz.clearPackages('total')
		DPK.setLabel('Velicina: [B][COLOR lime]0.0 B[/B][/COLOR]')
		TPK.setLabel('Datoteke: [B][COLOR lime]0[/B][/COLOR]')

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li obrisati predmemoriju, pakete i slicice?[/COLOR]' , nolabel='[B][COLOR white]Odustani[/COLOR][/B]',yeslabel='[B][COLOR white]Ocisti sve[/COLOR][/B]'):
		wiz.clearCache()
		wiz.clearPackages('total')
		clearThumb('total')
		TC.setLabel('Velicina: [B][COLOR lime]0.0 B[/B][/COLOR]')
		DC.setLabel('Velicina: [B][COLOR lime]0.0 B[/B][/COLOR]')
		DPK.setLabel('Velicina: [B][COLOR lime]0.0 B[/B][/COLOR]')
		TPK.setLabel('Datoteke: [B][COLOR lime]0[/B][/COLOR]')
		DTH.setLabel('Velicina: [B][COLOR lime]0.0 B[/B][/COLOR]')
		TTH.setLabel('Datoteke: [B][COLOR lime]0[/B][/COLOR]')
def clearThumb(type=None):
	latest = wiz.latestDB('Textures')
	if not type == None: choice = 1
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR lime]Zelite li obrisati %s i mapu minijatura' % (latest), "Ponovno ce se nakupiti prilikom sljedeceg pokretanja![/COLOR]", nolabel='[B][COLOR white]Odustani[/COLOR][/B]', yeslabel='[B][COLOR white]Obrisi[/COLOR][/B]')
	if choice == 1:
...
def list_update():
	...
	try:
		if window.getFocus() == buildlist:
			...
			buildtextbox.setLabel('[COLOR %s]Odabrani build: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR, name))
			...
			desctextbox.setText('[COLOR %s]Opis build-a: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
...
def BuildList():
	...
	if not BUILDFILE == 'http://' and not BUILDFILE == '':
		...
		PreviewButton = pyxbmct.Button('[COLOR %s][B]Pregled videozapisa[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(PreviewButton,20 , 20, 8, 8)
		window.connect(PreviewButton,lambda: buildVideo(Bname))
		
		InstallButton = pyxbmct.Button('[COLOR %s][B]Instaliraj[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(InstallButton,28 , 20, 8, 8)
		window.connect(InstallButton, lambda: buildWizard(Bname,'normal'))
		
		FreshStartButton = pyxbmct.Button('[COLOR %s][B]Svjeza instalacija[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(FreshStartButton,36 , 20, 8, 8)
		window.connect(FreshStartButton,lambda: buildWizard(Bname,'fresh'))
		
		ThemeButton = pyxbmct.Button('[COLOR %s][B]Instalacija teme[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		...
		buildname  = pyxbmct.Label('[COLOR %s]Trenutni build naziv: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildname))
		window.placeControl(buildname, 90, 2, 8, 25)
		
		buildversion  = pyxbmct.Label('[COLOR %s]Trenutna build verzija: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildversion))
		window.placeControl(buildversion, 95, 2, 8, 20)
		
		skinname  = pyxbmct.Label('[COLOR %s]Build tema naziv: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,defaultskinname))
		...
		errorinstall  = pyxbmct.Label('[COLOR %s]Pogreske tijekom instalacije: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,errors))
		window.placeControl(errorinstall, 95, 28, 8, 20)
		
		lastupdatchk  = pyxbmct.Label('[COLOR %s]Posljednja provjera azuriranja: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,lastbuildcheck))
...
def AddonList():
	...
	if not ADDONFILE == 'http://' and not ADDONFILE == '':
		...
		#buttons/objects
		InstallButtonA = pyxbmct.Button('[COLOR %s][B]Instaliraj[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def APKList():
	...
	if not APKFILE == 'http://' and not APKFILE == '':
		...
		InstallButtonAPK = pyxbmct.Button('[COLOR %s][B]Instaliraj[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def EmuList():
	...
	if not EMUAPKS == 'http://' and not EMUAPKS == '':
		...
		InstallButtonEMU = pyxbmct.Button('[COLOR %s][B]Instaliraj[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def RomList():
	...
	if not ROMPACK == 'http://' and not ROMPACK == '':
		...
		InstallButtonROM = pyxbmct.Button('[COLOR %s][B]Instaliraj[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def Un_Hide_Net():
	...
	INTER_IP.setLabel('[COLOR %s]Unutarnji IP: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,inter_ip))
	IP.setLabel('[COLOR %s]Vanjski IP:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,ip))
	CITY.setLabel('[COLOR %s]Grad:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,city))
	STATE.setLabel('[COLOR %s]Podrucje:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,state))
	COUNTRY.setLabel('[COLOR %s]Drzava: [/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,country))
	ISP.setLabel('[COLOR %s]Provajder:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,isp))
...
def Maint():
	...
	sysinfo_title =  pyxbmct.Label('[COLOR %s][B]INFORMACIJE SUSTAVA[/B][/COLOR]' % DES_T_COLOR)
	window.placeControl(sysinfo_title, 12, 31, 10, 15)
	version1 =  pyxbmct.Label('[COLOR %s]Verzija:[/COLOR] [COLOR %s]%s[/COLOR] - [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, codename, DESCOLOR, version))
	window.placeControl(version1, 18, 37, 10, 15)
	store = pyxbmct.Label('[B][COLOR %s]Prostor[/COLOR][/B]'% DESCOLOR)
	window.placeControl(store, 23, 39, 10, 10)
	rom_used=pyxbmct.Label('[COLOR %s]Zauzeto:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, storage_free))
	window.placeControl(rom_used, 28, 39, 10, 10)
	rom_free=pyxbmct.Label('[COLOR %s]Slobodno:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, storage_used))
	window.placeControl(rom_free, 32, 39, 10, 10)
	rom_total=pyxbmct.Label('[COLOR %s]Ukupno:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, storage_total))
	window.placeControl(rom_total, 37, 39, 10, 10)
	mem = pyxbmct.Label('[B][COLOR %s]Memorija[/COLOR][/B]' % DESCOLOR)
	window.placeControl(mem, 43, 39, 10, 10)
	### Hello, how are you
	ram_used=pyxbmct.Label('[COLOR %s]Zauzeto:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, ram_used))
	window.placeControl(ram_used, 48, 39, 10, 10)
	ram_free=pyxbmct.Label('[COLOR %s]Slobodno:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, ram_free))
	window.placeControl(ram_free, 53, 39, 10, 10)
	ram_total=pyxbmct.Label('[COLOR %s]Ukupno:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, ram_total))
	window.placeControl(ram_total, 58, 39, 10, 10)
	
	##addon info
	
	kodi = pyxbmct.Label('[COLOR %s]Naziv:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, name))
	window.placeControl(kodi, 17, 22, 10, 15)
	totalcount = len(picture) + len(music) + len(video) + len(programs) + len(scripts) + len(skins) + len(repos) 
	total = pyxbmct.Label('[COLOR %s]Broj dodatka: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, totalcount))
	window.placeControl(total, 22, 22, 10, 10)
	video=pyxbmct.Label('[COLOR %s]Video dodatci: [/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, str(len(video))))
	window.placeControl(video, 27, 22, 10, 10)
	program=pyxbmct.Label('[COLOR %s]Programski dodatci: [/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, str(len(programs))))
	window.placeControl(program, 33, 22, 10, 10)
	music=pyxbmct.Label('[COLOR %s]Glazbeni dodatci: [/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, str(len(music))))
	window.placeControl(music, 37, 22, 10, 10)
	picture=pyxbmct.Label('[COLOR %s]Slikovni dodatci: [/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, str(len(picture))))
	window.placeControl(picture, 42, 22, 10, 10)
	repos=pyxbmct.Label('[COLOR %s]Repozitoriji: [/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, str(len(repos))))
	window.placeControl(repos, 47, 22, 10, 10)
	skins=pyxbmct.Label('[COLOR %s]Teme: [/COLOR][COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, str(len(skins))))
	window.placeControl(skins, 52, 22, 10, 10)
	scripts=pyxbmct.Label('[COLOR %s]Skripte / Moduli: [/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR, str(len(scripts))))
	...
	netinfo_title =  pyxbmct.Label('[COLOR %s][B]INTERNET INFORMACIJE[/B][/COLOR]'% DES_T_COLOR)
	window.placeControl(netinfo_title, 12, 7, 10, 20)
	MAC = pyxbmct.Label('[COLOR %s]Mac:[/COLOR] [COLOR %s]Skriveno[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	window.placeControl(MAC, 18 , 1 ,  10, 18)
	INTER_IP = pyxbmct.Label('[COLOR %s]Unutarnji IP: [COLOR %s]Skriveno[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	window.placeControl(INTER_IP, 23 , 1 ,  10, 18)
	IP = pyxbmct.Label('[COLOR %s]Vanjski IP:[/COLOR] [COLOR %s]Skriveno[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	window.placeControl(IP, 28 , 1 ,  10, 18)
	CITY = pyxbmct.Label('[COLOR %s]Grad:[/COLOR] [COLOR %s]Skriveno[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	window.placeControl(CITY, 33 , 1 ,  10, 18)
	STATE = pyxbmct.Label('[COLOR %s]Podrucje:[/COLOR] [COLOR %s]Skriveno[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	window.placeControl(STATE, 38 , 1 ,  10, 18)
	COUNTRY = pyxbmct.Label('[COLOR %s]Drzava:[/COLOR] [COLOR %s]Skriveno[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	window.placeControl(COUNTRY, 43 , 1 ,  10, 18)
	ISP = pyxbmct.Label('[COLOR %s]Provajder:[/COLOR] [COLOR %s]Skriveno[/COLOR]' % (DES_T_COLOR, DESCOLOR))
	...
	#buttons#
	total_clean_button = pyxbmct.Button('[COLOR %s]Zajednicko ciscenje[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_clean_button, 72 , 21 ,  9, 8)
	window.connect(total_clean_button,lambda: totalClean())
	### I'm good, u?
	total_cache_button = pyxbmct.Button('[COLOR %s]Brisanje predmemorije[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_cache_button, 87 , 21 ,  9, 9)
	window.connect(total_cache_button,lambda: clearCache())
	
	total_packages_button = pyxbmct.Button('[COLOR %s]Brisanje paketa[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_packages_button, 87 , 31 ,  9, 9)
	window.connect(total_packages_button,lambda: clearPackages())
	
	total_thumbnails_button = pyxbmct.Button('[COLOR %s]Brisanje minijatura[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(total_thumbnails_button, 87 , 41 ,  9, 9)
	window.connect(total_thumbnails_button, lambda: clearThumb(type=None))
	
	speedtest_button = pyxbmct.Button('[COLOR %s]Testiranje brzine[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(speedtest_button, 58 , 1 ,  9, 8)
	window.connect(speedtest_button, lambda: runspeedtest())
	
	un_hide_net = pyxbmct.Button('[COLOR %s]Internet informacije[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(un_hide_net, 58, 10, 9, 8)
	window.connect(un_hide_net, lambda: Un_Hide_Net())
	
	trigger_title =  pyxbmct.Label('[COLOR %s]Promijena aktivnosti[/COLOR]'% DES_T_COLOR)
	window.placeControl(trigger_title, 72 , 30 , 1, 11)
	
	settings_button1 = pyxbmct.Button('[COLOR %s]Postavke[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def BackRes():
	...
	last = str(FAVSsave) if not FAVSsave == '' else 'Favoriti jos nisu spremljeni.'
	#loc info#
	
	favs = pyxbmct.Label('[B][COLOR %s]Zadnja pohrana:[/COLOR] [COLOR %s]%s[/COLOR][/B]' % (DES_T_COLOR, DESCOLOR,str(last)))
	window.placeControl(favs, 14, 3, 10, 30)
	
	backuploc = pyxbmct.Label('[B][COLOR %s]Lokacija: [COLOR %s]%s[/COLOR][/B]' % (DES_T_COLOR, DESCOLOR,MYBUILDS))
	window.placeControl(backuploc, 22, 3, 10, 30)
	
	#backup#
	Backup = pyxbmct.Label('[B][COLOR %s]Sigurnosne kopije:[/COLOR][/B]' % DES_T_COLOR)
	...
	backupaddonpack_button = pyxbmct.Button('[COLOR %s] Paket dodataka[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	backup_addondata_button = pyxbmct.Button('[COLOR %s] Podaci o dodatku[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	savefav_button = pyxbmct.Button('[COLOR %s] Favoriti[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(savefav_button, 82, 3, 10, 10)
	window.connect(savefav_button,lambda: wiz.BACKUPFAV())
	
	#restore#
	restore_title = pyxbmct.Label('[B][COLOR %s]Alati:[/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(restore_title, 32, 20, 10, 10)
	
	restore_build_button = pyxbmct.Button('[COLOR %s]Build[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	### Nope not here
	restore_addondata_button = pyxbmct.Button('[COLOR %s]Podaci o dodatku[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	restorefav_button = pyxbmct.Button('[COLOR %s]Favoriti[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(restorefav_button, 72, 20, 10, 10)
	window.connect(restorefav_button,lambda: wiz.RESTOREFAV())
	
	#clear backups#
	delete_title = pyxbmct.Label('[B][COLOR white]Brisanje:[/COLOR][/B]')
	window.placeControl(delete_title, 54, 37, 10, 10)
	
	clearfav_button = pyxbmct.Button('[COLOR %s]Brisanje favorita[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(clearfav_button, 64, 37, 10, 10)
	window.connect(clearfav_button,lambda: wiz.DELFAV())
	
	clear_backup_button = pyxbmct.Button('[COLOR %s]Brisanje sigurnosnih kopija[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(clear_backup_button, 74, 37, 10, 10)
	window.connect(clear_backup_button,lambda: wiz.cleanupBackup())
	
	#settings
	set_title = pyxbmct.Label('[B][COLOR %s]Postavke:[/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(set_title, 32, 37, 10, 10)
	
	settings_button = pyxbmct.Button('[COLOR %s]Postavke[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def Tools():
	...
	Log_title = pyxbmct.Label('[B][COLOR %s]Alati za biljezenje[/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(Log_title, 15, 4, 10, 10)
	
	Log_errors = pyxbmct.Label('[COLOR %s][B]Pogreske u zapisniku:[/B][/COLOR] %s' % (OTHER_BUTTONS_TEXT, log_tools()))
	window.placeControl(Log_errors, 22, 4, 10, 15)
	## uhhh not here
	view_error_button = pyxbmct.Button('[COLOR %s]Pregled pogresaka[/COLOR]'% OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	full_log_button = pyxbmct.Button('[COLOR %s]Pregled cijelog zapisnika[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(full_log_button, 40, 4, 9, 9)
	window.connect(full_log_button,lambda :LogViewer())
	
	upload_log_button = pyxbmct.Button('[COLOR %s]Ucitaj cijeli zapisnik[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	Addon = pyxbmct.Label('[B][COLOR %s]Alati za dodatke[/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(Addon, 69, 21, 9, 9)
	#buttons#
	scan = pyxbmct.Label('[B][COLOR %s]Skeniranje:[/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(scan, 75, 4, 9, 10)
	
	checksources_button = pyxbmct.Button('[COLOR %s]Prekinuti izvori[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(checksources_button, 82, 3, 9, 10)
	window.connect(checksources_button,lambda: wiz.checkSources())
	
	checkrepos_button = pyxbmct.Button('[COLOR %s]Prekinuti repozitoriji[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(checkrepos_button, 92, 3, 9, 10)
	window.connect(checkrepos_button,lambda: wiz.checkRepos())
	
	fix = pyxbmct.Label('[B][COLOR %s]Prisilni popravak:[/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(fix, 75, 16, 9, 10)
	
	forceupdate_button = pyxbmct.Button('[COLOR %s]Azuriranje dodataka[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(forceupdate_button, 82, 15, 9, 9)
	window.connect(forceupdate_button,lambda: wiz.forceUpdate())
	
	fixaddonupdate_button = pyxbmct.Button('[COLOR %s]Ne azurirani dodatci[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(fixaddonupdate_button, 92, 15, 9, 9)
	window.connect(fixaddonupdate_button,lambda: fixaddonupdate)
	### I see you looking
	delet = pyxbmct.Label('[B][COLOR %s]Brisanje: [/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(delet, 75, 28, 9, 10)
	
	removeaddons_button = pyxbmct.Button('[COLOR %s]Brisanje odabranih dodataka[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(removeaddons_button, 82, 26, 9, 9)
	window.connect(removeaddons_button,lambda: removeAddonMenu)
	
	removeaddondata_all_button = pyxbmct.Button('[COLOR %s]Svi podaci o dodatcima[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	delet1 = pyxbmct.Label('[B][COLOR %s]Brisanje: [/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(delet1, 75, 39, 9, 10)
	
	removeaddondata_u_button = pyxbmct.Button('[COLOR %s]Deinstaliranje mape[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	removeaddondata_e_button = pyxbmct.Button('[COLOR %s]Prazne mape[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	WhiteList = pyxbmct.Label('[B][COLOR %s]Alati sigurne liste: [/COLOR][/B]' % DES_T_COLOR)
	window.placeControl(WhiteList, 15, 36, 9, 9)
	
	whitelist_edit_button = pyxbmct.Button('[COLOR %s]Sigurna lista: Uredi[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(whitelist_edit_button, 22, 36, 9, 9)
	window.connect(whitelist_edit_button,lambda: wiz.whiteList('edit'))
	
	whitelist_view_button = pyxbmct.Button('[COLOR %s]Sigurna lista: Pregled[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(whitelist_view_button, 31, 36, 9, 9)
	window.connect(whitelist_view_button,lambda: wiz.whiteList('view'))
	
	whitelist_clear_button = pyxbmct.Button('[COLOR %s]Sigurna lista: Ciscenje[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(whitelist_clear_button, 40, 36, 9, 9)
	window.connect(whitelist_clear_button,lambda: wiz.whiteList('clear'))
	### It's not here
	whitelist_import_button = pyxbmct.Button('[COLOR %s]Sigurna lista: Uvoz[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(whitelist_import_button, 49, 36, 9, 9)
	window.connect(whitelist_import_button,lambda: wiz.whiteList('Import'))
	
	whitelist_export_button = pyxbmct.Button('[COLOR %s]Sigurna lista: Izvoz[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	...
	Advan = pyxbmct.Label('[B][COLOR %s]Alati za napredne postavke[/COLOR][/B]'% DES_T_COLOR)
	window.placeControl(Advan, 15, 20, 9, 13)
	#buttons#
	autoadvanced_buttonQ = pyxbmct.Button('[COLOR %s]Brza konfiguracija[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(autoadvanced_buttonQ, 22, 21, 9, 9)
	window.connect(autoadvanced_buttonQ,lambda: notify.autoConfig2())
	
	autoadvanced_button = pyxbmct.Button('[COLOR %s]Puna konfiguracija[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(autoadvanced_button, 31, 21, 9, 9)
	window.connect(autoadvanced_button,lambda: notify.autoConfig())
	##Wait it's here!!
	currentsettings_button = pyxbmct.Button('[COLOR %s]Trenutne postavke[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
	window.placeControl(currentsettings_button, 40, 21, 9, 9)
	window.connect(currentsettings_button,lambda: viewAdvanced())
	
	removeadvanced_button = pyxbmct.Button('[COLOR %s]Brisanje postavki[/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
def Installables():
	...
	AddonButton = pyxbmct.Button('[COLOR %s][B]Wizardi[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
	window.placeControl(AddonButton,12 , 9,  7, 8)
	window.connect(AddonButton, lambda: AddonList())
	
	APKButton = pyxbmct.Button('[COLOR %s][B]Aplikacije[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
...
wiz_ver =  pyxbmct.Label('[COLOR %s]Verzija: [COLOR %s][B]%s[/B][/COLOR]' % (uservar.VERTITLE_COLOR,uservar.VER_NUMBER_COLOR,VERSION))
...
#buttons/objects
BuildsButton= pyxbmct.Button('[COLOR %s][B]Buildovi[/B][/COLOR]' % MAIN_BUTTONS_TEXT, focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(BuildsButton,-2 , 1 , 13, 8)
window.connect(BuildsButton, lambda: BuildList())

MaintButton = pyxbmct.Button('[COLOR %s][B]Odrzavanje[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(MaintButton,2 , 9 , 9, 8)
window.connect(MaintButton, lambda: Maint())

BackResButton = pyxbmct.Button('[COLOR %s][B]Kopija/Vracanje[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(BackResButton,2 , 17, 9, 8)
window.connect(BackResButton, lambda: BackRes())

ToolsButton = pyxbmct.Button('[COLOR %s][B]Alati[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(ToolsButton,2 , 25, 9, 8)
window.connect(ToolsButton, lambda: Tools())

InstallablesButton = pyxbmct.Button('[COLOR %s][B]Mogucnosti[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(InstallablesButton,2 , 33, 9, 8)
window.connect(InstallablesButton, lambda: Installables())

CloseButton = pyxbmct.Button('[COLOR %s][B]Zatvori[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
