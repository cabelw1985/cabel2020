...
#                                                                              #
#  Translated and modified by Cabel 07.2018             #
#                                                                              #
################################################################################
...
def writeAdvanced(name, url):
	...
	if ADVANCEDWORKING == True:
		...
		if choice == 1:
			...
			f.write(file)
			f.close()
			DIALOG.ok(ADDONTITLE, '[COLOR %s]AdvancedSettings.xml wurde erfolgreich geschrieben.  Klicke [COLOR lime][B]OK[/COLOR][/B] um Cabel zum schliessen zu zwingen.[/COLOR]' % COLOR2)
			wiz.killxbmc(True)
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]Cabel schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR lime]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR red]Cabel schliessen[/COLOR][/B]")
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		...
		if yes:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Der GuiFix wurde installiert.  Willst du das Profil neu laden oder Cabel schliessen?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]Cabel schliessen[/COLOR][/B]")
			if todo == 1: wiz.reloadFix()
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Um die Anderungen zu speichern musst du Cabel jetzt schliessen, klicke OK um Cabel zu schliessen[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]Cabel schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]Cabel schliessen[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du Cabel resetten" % COLOR2, "auf standard Einstellungen", "bevor du das lokale Backup installierst?[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du Cabel resetten" % COLOR2, "auf standard Einstellungen", "bevor du [COLOR %s]%s[/COLOR] installierst?" % (COLOR1, install), nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du Cabel resetten" % COLOR2, "auf standard Einstellungen?[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
